...
function heuristic_rate_guess(nper, pmt, pv, fv)
    if pv-fv ==0
        return 0
    else
...
function rate(
    nper, pmt, pv, fv=0.0, type=0, guess=nothing; tol=1e-10, max_iter=100
)::Float64
    # Ensure that type is either 0 or 1
    if type != 0 && type != 1
        error("type must be 0 (end of period) or 1 (beginning of period)")
    end

    # Iterative calculation using Newton's method
    rate_guess = if guess !== nothing
        guess
    else
        heuristic_rate_guess(nper, pmt, pv, fv)
    end

    for i in 1:max_iter
        # Calculate the current guess's present value
        if rate_guess == 0.0
            f_value = pv + pmt * nper + fv
        else
            f_value =
                pv * (1 + rate_guess)^nper +
                pmt * (1 + rate_guess * type) * ((1 + rate_guess)^nper - 1) / rate_guess +
                fv
        end
        ...
        if rate_guess == 0.0
            ...
            f_prime =
                pv * nper * (1 + rate_guess)^(nper - 1) +
                pmt * (1 + rate_guess * type) * ((1 + rate_guess)^nper - 1) / rate_guess +
                pmt * nper * (1 + rate_guess * type) * (1 + rate_guess)^(nper - 1)
        end

        # Newton's step
        new_rate = rate_guess - f_value / f_prime

        # Check for convergence
        if abs(new_rate - rate_guess) < tol
            return new_rate
        end
    ...
    return error("did not converge within $max_iter iterations")
end